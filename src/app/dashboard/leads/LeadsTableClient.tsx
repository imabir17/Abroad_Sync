  ...
  return (
    <div className="neo-raised overflow-hidden">
      ...
      {isAdminOrManager && selectedLeadIds.length > 0 && (
        <div className="bg-[#DCE3ED] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-3">
            <select 
              ...
              className="px-3 py-2 bg-white border border-gray-200 text-xs font-bold text-gray-600 rounded-xl outline-none focus:bg-gray-50 border border-gray-200 transition-all cursor-pointer"
            >
            ...
            <button 
              ...
              className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl shadow-md hover:shadow-[9px_9px_20px_rgba(51,63,194,0.35)] active:translate-y-0.5 disabled:opacity-50 transition-all duration-150 flex items-center gap-1.5"
            >
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-[#DCE3ED] border-b border-gray-200 text-gray-600 text-[10px] font-bold uppercase tracking-wider">
              {isAdminOrManager && (
          ...
          <tbody className="divide-y divide-[#AEB9C9]/20">
              ...
              <tr key={lead.id} className="hover:bg-[#DCE3ED]/20 transition-colors group">
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-gray-900">{lead.fullName}</span>
                    <span className="text-[10px] text-gray-400 mt-0.5">{lead.phone ? lead.phone : lead.email}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  ...
                  <div className="flex items-center justify-end gap-2.5">
                    {lead.phone && (
                      <a 
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white border border-gray-200 text-xs font-bold text-[#25D366] hover:bg-gray-50 border border-gray-200 transition-all"
                    ...
                    <Link 
                      ...
                      className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white border border-gray-200 text-xs font-bold text-[#4855E4] hover:bg-gray-50 border border-gray-200 transition-all whitespace-nowrap"
                    >
            ...
            {activeLeads.length === 0 && (
              <tr>
                <td colSpan={isAdminOrManager ? 7 : 6} className="px-6 py-12 text-center text-xs font-bold text-gray-400">
      ...
      <div className="block md:hidden space-y-4 p-2">
        {activeLeads.map((lead, i) => {
          ...
          return (
            <div 
              key={lead.id} 
              className="bg-white border border-gray-200 rounded-2xl p-4 space-y-3.5 border border-gray-200"
            >
              {/* Header: Name, Country, Avatar */}
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center text-xs font-bold shrink-0 shadow-sm">
                    {getInitials(lead.fullName)}
                  </div>
                  <div className="min-w-0">
                    <h4 className="text-xs font-bold text-gray-900 truncate">{lead.fullName}</h4>
              ...
              <div className="flex items-center justify-between text-xs pt-2 border-t border-gray-200">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[9px] text-gray-400 font-semibold">Lead Rating</span>
                  <StarRating rating={lead.rating} editable={false} size={11} />
                </div>
                <div className="text-right">
                  <span className="text-[9px] text-gray-400 block font-semibold">Counselor</span>
              ...
              <div className="flex items-center gap-3 pt-2">
                {lead.phone && (
                  <a 
                    href={`tel:${lead.phone}`}
                    className="flex-1 h-10 bg-white border border-gray-200 text-emerald-600 rounded-xl hover:bg-gray-50 border border-gray-200 transition-all flex items-center justify-center gap-2 text-xs font-bold"
                  >
                ...
                {lead.phone && (
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 h-10 bg-white border border-gray-200 text-[#25D366] rounded-xl hover:bg-gray-50 border border-gray-200 transition-all flex items-center justify-center gap-1.5 text-xs font-bold"
                  >
                ...
                <Link 
                  href={`/dashboard/leads/${lead.id}`}
                  className="flex-1 h-10 bg-blue-600 text-white rounded-xl shadow-md active:translate-y-0.5 transition-all flex items-center justify-center gap-1.5 text-xs font-bold"
                >
                  <span>View</span>
                </Link>
              </div>
        ...
        {activeLeads.length === 0 && (
          <div className="neo-raised p-8 text-center text-xs font-bold text-gray-400">
